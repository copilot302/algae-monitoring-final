...
const char* ssid = "PLDTHOMEFIBR5Ke5a";
const char* password = "PLDTWIFIZHHQ5";
const char* serverUrl = "http://192.168.100.7:5000/api/sensor-data";

// --- Device Configuration (UPDATE FOR EACH DEVICE) ---
const char* deviceId = "POND-01";        // Unique ID: POND-01, POND-02, TANK-A, etc.
const char* deviceName = "Main Pond";    // Display name: "Main Pond", "Tank A", etc.
...
void sendDataToServer(float temp, float pH, float ecVal, float turbidity) {
    ...
    doc["ec"] = round(ecVal * 1000 * 100) / 100.0;  // Convert ms/cm to μS/cm
...
void loop()
{
    ...
    if (now - last[3] >= intervals[3]) 
    {
        ...
        if (!ecPhCalibrationActive && !turbidityCalibrationMode)
        {
            ...
            // NEW CORRECT CODE
// Convert Raw Bits to Millivolts (GAIN_ONE: 1 bit = 0.125mV)
            ecVoltage = ads.readADC_SingleEnded(0) * 0.125; 
            phVoltage = ads.readADC_SingleEnded(1) * 0.125; 
